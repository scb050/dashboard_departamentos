# app.py — Mapa interactivo PyDeck sin pandas
...
def color_for_value(v):
    if v is None:
        return [230, 230, 230, 180]
    for i in range(5):
        if qs[i] <= v <= qs[i + 1]:
            return palette[i] + [200]
    return palette[-1] + [200]

# inyectar color/tooltip
...
# === VISTA FIJA SOBRE COLOMBIA (evita quedarnos fuera de escena) ===
lat, lon, zoom = 4.6, -74.3, 4.5  # céntrate en Colombia

# capa GeoJSON
layer = pdk.Layer(
    "GeoJsonLayer",
    data=gj,                  # pasamos el dict directamente
    stroked=True,
    filled=True,
...
# IMPORTANTE: desactivar mapa base (sin token)
deck = pdk.Deck(
    layers=[layer],
